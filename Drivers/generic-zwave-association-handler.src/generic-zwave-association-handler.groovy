/**
 *
 *  Generic Z-Wave Association Handler
 *   
...
metadata {
	definition (name: "Generic Z-Wave Association Handler", namespace: "erocm123", author: "Eric Maycock") {
...
def associateGroup(group, nodes){
   state."desiredAssociation${group}" = nodes
   log.debug state."desiredAssociation${group}"
}
...
private processAssociations(){
   ...
   for (int i = 1; i <=5; i++){
      if(state."actualAssociation${i}" != null){
         if(state."desiredAssociation${i}" != null) {
            def refreshGroup = false
            (state."desiredAssociation${i}" - state."actualAssociation${i}"*.toString()).each {
            ...
            (state."actualAssociation${i}"*.toString() - state."desiredAssociation${i}").each {
               if(it.toInteger() != 1) {
                  log.debug "Removing node $it from group $i"
                  cmds << zwave.associationV2.associationRemove(groupingIdentifier:i, nodeId:it.toInteger()).format()
                  refreshGroup = true
               }
            }
            if (refreshGroup == true) cmds << zwave.associationV2.associationGet(groupingIdentifier:i).format()
            else log.debug "There are no association actions to complete"
         }
      } else {
