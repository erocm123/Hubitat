...
metadata {
	...
	simulator {
		...
		for (int i = 0; i <= 100; i += 20) {
			status "temperature ${i}F": new hubitat.zwave.Zwave().sensorMultilevelV5.sensorMultilevelReport(
				scaledSensorValue: i, precision: 1, sensorType: 1, scale: 1).incomingMessage()
		}
		for (int i = 0; i <= 100; i += 20) {
			status "luminance ${i} %": new hubitat.zwave.Zwave().sensorMultilevelV5.sensorMultilevelReport(
