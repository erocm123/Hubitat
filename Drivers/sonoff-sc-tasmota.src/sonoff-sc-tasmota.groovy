...
metadata {
	definition (name: "Sonoff SC - Tasmota", namespace: "erocm123", author: "Eric Maycock", vid:"generic-switch") {
        capability "Actuator"
		capability "Switch"
		capability "Refresh"
		capability "Sensor"
...
//04:09:34 RSL: SENSOR = {"Time":"2019-04-04T04:09:33","SonoffSC":{"Temperature":20.0,"Humidity":21.0,"Light":30,"Noise":40,"AirQuality":100},"TempUnit":"C"}
    ...
    if (result.containsKey("SonoffSC")) {
        if (result.SonoffSC.containsKey("Humidity"))
            log.debug "Humidity: ${getAdjustedHumidity(state.realHumidity? state.realHumidity:0)}"
            state.realHumidity = Math.round((result.SonoffSC.Humidity as Double) * 100) / 100
            events << createEvent(name: "humidity", value:"${getAdjustedHumidity(state.realHumidity)}", unit:"%")
        if (result.SonoffSC.containsKey("Temperature")) {
            log.debug "Temperature: ${getAdjustedTemp(state.realTemperature? state.realTemperature:0)}"
            state.realTemperature = convertTemperatureIfNeeded(result.SonoffSC.Temperature.toFloat(), result.TempUnit, 1)
            events << createEvent(name:"temperature", value:"${getAdjustedTemp(state.realTemperature)}", unit:"${location.temperatureScale}")
        }
        if (result.SonoffSC.containsKey("AirQuality")) {
            log.debug "AirQuality: $result.SonoffSC.AirQuality"
            events << createEvent(name: "airQuality", value: result.SonoffSC.AirQuality, unit: "%")
        }
        if (result.SonoffSC.containsKey("Light")) {
            log.debug "Light: $result.SonoffSC.Light"
            events << createEvent(name: "illuminance", value: result.SonoffSC.Light, unit: "%")
        }
        if (result.SonoffSC.containsKey("Noise")) {
            log.debug "Noise: $result.SonoffSC.Noise"
            events << createEvent(name: "noise", value: result.SonoffSC.Noise, unit: "%")
        }
