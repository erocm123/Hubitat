...
metadata {
	definition (name: "Sonoff S31 - Tasmota", namespace: "erocm123", author: "Eric Maycock", vid:"generic-switch-power-energy") {
        capability "Actuator"
		capability "Switch"
		capability "Refresh"
		capability "Sensor"
...
def parse(description) {
    ...
    if (result.containsKey("A")) {
        events << createEvent(name: "amperage", value: result.A, unit: "A")
    }
    if (result.containsKey("W")) {
        events << createEvent(name: "power", value: result.W, unit: "W")
    }
    } else {
