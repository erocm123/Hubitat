 ...
 */
 ...
 metadata {
	definition (name: "Aeon Multisensor", namespace: "erocm123", author: "Eric Maycock") {
...
next WakeUp (60 minutes).",
	...
	simulator {
		...
		for (int i = 0; i <= 100; i += 20) {
			status "RH ${i}%": new hubitat.zwave.Zwave().securityV1.securityMessageEncapsulation().encapsulate(
				new hubitat.zwave.Zwave().sensorMultilevelV2.sensorMultilevelReport(
                	scaledSensorValue: i,
                    sensorType: 5
            	)
