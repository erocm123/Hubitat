...
def zwaveEvent(hubitat.zwave.commands.wakeupv1.WakeUpIntervalReport cmd, results)
{
    log.debug cmd
...
def zwaveEvent(hubitat.zwave.commands.alarmv2.AlarmReport cmd, results) {
	...
	} else if (cmd.zwaveAlarmType == hubitat.zwave.commands.alarmv2.AlarmReport.ZWAVE_ALARM_TYPE_BURGLAR) {
	...
	} else if (cmd.zwaveAlarmType == hubitat.zwave.commands.alarmv2.AlarmReport.ZWAVE_ALARM_TYPE_POWER_MANAGEMENT) {
...
def zwaveEvent(hubitat.zwave.commands.sensormultilevelv5.SensorMultilevelReport cmd, results)
{
    log.debug cmd
    def map = [:]
